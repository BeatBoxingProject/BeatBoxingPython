...
\# core/config.py  
CAM\_IP\_1 \= "192.168.1.101"   
CAM\_IP\_2 \= "192.168.1.102"
...
python tools/align\_cameras.py
...
* **Result:** This will save a stereo\_calib.yml file into the data/ folder.

### **3\. Color Tuning**

Run the tuner to isolate your glove from the background.  
python tools/color\_tuner.py
...
Because the cameras are angled down at 50°, a raw Z-depth calculation would result in the coordinate moving "down" as you punch "forward". The apply\_tilt\_correction function (in core/vision.py) uses a rotation matrix to correct this:  
$$y\_{new} \= y \\cdot \\cos(\\theta) \- z \\cdot \\sin(\\theta)$$$$z\_{new} \= y \\cdot \\sin(\\theta) \+ z \\cdot \\cos(\\theta)$$
